...
void AcheterPropriete(t_caseDePlateau PLATEAUS[LONGUEUR_PLATEAU], t_joueur JOUEURS[4], int indiceJoueur,int indicePlateau)
{
    ...
    strcpy(JOUEURS[indiceJoueur].POSSESSIONSNOMS[JOUEURS[indiceJoueur].nombreDePropriete], PLATEAUS[indicePlateau].nomDeCase);

    ///Virement bancaire
    JOUEURS[indiceJoueur].soldeBancaire -= PLATEAUS[indicePlateau].prixAchat; //on soustrait au solde du joueur, le prix d'achat
