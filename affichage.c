...
void afficherPlateau(CaseDePlateau TAB[NOMBRE_DE_CASES_DU_PLATEAU], Joueur JOUEURS[NOMBRE_DE_JOUEURS] )
{
    int i=0;
    for (i=0; i<NOMBRE_DE_CASES_DU_PLATEAU; i++)        //affiche mon tableau sans respecter les 4 lignes et 8 colonnes du plateau
    {
        printf("%d", TAB[i].numeroCase);
        for(int j=0; j<NOMBRE_DE_JOUEURS; j++)
        {
            if (JOUEURS[j].positionPlateau == TAB[i].numeroCase){
